    ...
    public class Project {
        ...
        private string MakeTree(string dirGuid) {
            ...
            if(ProjectType == ProjectTypeEnum.Maven) {
                ExecuteCommand(CLI_RM, "tree.json", dirGuid);
                string outputPath = AppDomain.CurrentDomain.BaseDirectory + dirGuid + "/tree.json";
                ExecuteCommand("mvn", $"org.apache.maven.plugins:maven-dependency-plugin:3.8.0:tree -DoutputFile={outputPath} -DoutputType=json -Dscope=runtime", dirGuid);
                return AppDomain.CurrentDomain.BaseDirectory + dirGuid + "/tree.json";
            }
